...
class Pastas(QMainWindow, Ui_mainWindow):
    ...
    def inserir_arquivo(self):
        ...
        if pedido:
            ...
            resposta = agenda.analisa_se_pedido(pedido)
    ...
    def estornar_liquidar(self, pedido, liquidado):
        if pedido:
            agenda = BDArquivos('agenda.db')
    ...
    def excluir_pedido(self):
        ...
        if not pedido:
            ...
            if rm == 16384:
                agenda = BDArquivos('agenda.db')
                resultado = agenda.excluir_pedido_bd(pedido)
                if resultado:
    ...
    def log_in(self):
        ...
        def fecha():
            ...
            global logado
            global direito
            usuario = pegar_usuario.get()
            senha = pegar_senha.get()
            dominio = dominio_get

            agenda = BDArquivos('agenda.db')
            resultado = agenda.verificar_login_db(usuario, senha, dominio)
            if resultado:
